...
hwhite=np.fft.irfft(hft_win/sig)
tft=np.fft.rfft(tp)
twhite=np.fft.irfft(tft/sig)
#plt.clf();plt.plot(hwhite);plt.show()
plt.clf();plt.plot(twhite);plt.show()

white_ps=np.fft.rfft(hwhite)
plt.clf()
plt.loglog(np.abs(white_ps))
plt.show()
